  ...
  const fetchStatusData = async () => {
    try {
      ...
      const filteredData = data.filter(
        (status: ActivityStatus) => status.is_active !== false
      );
    ...
    try {
      ...
      const transformedData = data.map((item: any) => ({
        ...
        assignee: item.users
          ...
          : "Unassigned",
        activity_priorities: item.activity_priorities || {
          title: item.status,
          color: "primary",
        },
        users: item.users,
      ...
      const usersWithActivities = new Set(
        transformedData.map((item: any) => item.user_id)
      );
    ...
    if (props.isPlaceholder) {
      return (
        <div className="card-template">
          <div
            className="card border-0 border-light"
          ...
          <div className="card-body" style={{ padding: "20px" }}>
            ...
            >
              <div className="d-inline-block mr-1" key={0}>
                <div
                  className={`badge badge-${props.activity_priorities?.color || "primary"}`}
                >
              ...
              {props.tags &&
                props.tags.split(",").map((tag2: any, idx: number) => (
                  ...
                  >
                    <div
                      className={`badge ${badgeData[idx % badgeData.length]}`}
                    >
                      {tag2.trim()}
                    </div>
                  </div>
            ...
            <div className="card-text">
              Start: {formatDate(props.activity_start)}
            </div>
            <div className="card-text">
              End: {formatDate(props.activity_end)}
            </div>
            <div className="gap-2 mt-3 d-flex flex-column">
              <Button
