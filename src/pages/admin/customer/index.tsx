...
import { Container, Row, Col, Card, CardBody, CardHeader, Button, Table, Badge, Input, Modal, ModalHeader, ModalBody, ModalFooter, Form, FormGroup, Label, Alert, Spinner } from 'reactstrap';
import { Search, Plus, Edit, Trash2, Users, Home, Eye, UserPlus, X } from 'react-feather';
import { toast } from 'react-toastify';
import Breadcrumbs from "CommonElements/Breadcrumbs";
import { Company, CompanyStats, CompanyFormData, CompanyMember } from '../../../../Types/CompanyType';
...
const CompanyManagement = () => {
  ...
  const handleUpdateLeader = async () => {
    ...
    try {
      await CompanyService.updateCompanyLeader(selectedCompany.id, selectedUserId);
  ...
  const handleAssignMember = async () => {
    ...
    try {
      await CompanyService.assignUserToCompany(selectedUserId, selectedCompany.id);
  ...
  const availableLeaders = allUsers.filter(user => 
    !selectedCompany?.members?.some(member => member.id === user.id)
  );
  ...
  return (
    <div className="page-body">
      ...
      <Container fluid>
              ...
              <CardBody>
                ...
                <Row className="mb-3">
                  <Col md={6}>
                    <div className="position-relative">
                      <Search size={16} className="position-absolute" style={{ left: '10px', top: '50%', transform: 'translateY(-50%)', color: '#6c757d' }} />
                      <Input
                        type="text"
                  ...
                  <tbody>
                    {filteredCompanies.map((company) => (
                      <tr key={company.id}>
                        ...
                        <td>
                          <Badge color="secondary">{company.industry || 'N/A'}</Badge>
                        </td>
                        <td>
                          {company.leader ? (
                            <div className="d-flex align-items-center">
                              ...
                              <span className="small">
                                {company.leader.first_name} {company.leader.last_name}
                              </span>
                            </div>
                          ) : (
                        ...
                        <td>
                          <Badge color={company.is_active ? 'success' : 'danger'}>
                ...
                {filteredCompanies.length === 0 && (
                  <Alert color="info" className="text-center">
                    No companies found. {searchTerm ? 'Try adjusting your search.' : 'Create your first company to get started.'}
                  </Alert>
                )}
              </CardBody>
            </Card>
        ...
        {/* Company Form Modal */}
        <Modal isOpen={showCompanyModal} toggle={() => setShowCompanyModal(false)} size="lg">
          ...
          <Form onSubmit={handleSubmitCompany}>
                ...
                <Col md={6}>
                  <FormGroup>
                    ...
                    <Input
                      ...
                      onChange={(e) => setCompanyForm({ ...companyForm, leader_id: e.target.value })}
                    >
            ...
            <ModalFooter>
              <Button color="secondary" onClick={() => setShowCompanyModal(false)}>
                Cancel
              </Button>
              <Button color="primary" type="submit" disabled={modalLoading}>
                {modalLoading ? <Spinner size="sm" /> : (isEditing ? 'Update' : 'Create')}
              </Button>
            </ModalFooter>
          </Form>
        </Modal>
                    ...
                    <CardBody>
                      <div className="d-flex align-items-center mb-3">
                        ...
                        <div>
                          ...
                          <p className="text-muted mb-0">{selectedCompany.description}</p>
                        </div>
                      </div>
                      
                      <Row>
                        <Col md={6}>
                          <p><strong>Industry:</strong> {selectedCompany.industry || 'N/A'}</p>
                        ...
                        <Col md={6}>
                          <p><strong>Founded:</strong> {selectedCompany.founded_date || 'N/A'}</p>
                          <p><strong>Address:</strong> {selectedCompany.address || 'N/A'}</p>
                          <p><strong>City:</strong> {selectedCompany.city || 'N/A'}</p>
                          <p><strong>Country:</strong> {selectedCompany.country || 'N/A'}</p>
                        </Col>
                      </Row>
                    </CardBody>
                  </Card>
                          ...
                          <div className="d-flex align-items-center">
                            ...
                            <div>
                              <small className="fw-bold">
                                {selectedCompany.leader.first_name} {selectedCompany.leader.last_name}
                              </small>
                              <br />
                              <small className="text-muted">{selectedCompany.leader.email}</small>
                            </div>
                          </div>
                          <Button
                            color="danger"
                            size="sm"
                            onClick={() => handleRemoveLeader(selectedCompany.id)}
                          >
                            <X size={12} />
                          </Button>
                        </div>
        ...
        {/* Update Leader Modal */}
        <Modal isOpen={showLeaderModal} toggle={() => setShowLeaderModal(false)}>
          ...
          <ModalFooter>
            ...
            <Button color="primary" onClick={handleUpdateLeader} disabled={!selectedUserId}>
              Update Leader
            </Button>
          </ModalFooter>
        </Modal>
        ...
        {/* Add Member Modal */}
        <Modal isOpen={showMemberModal} toggle={() => setShowMemberModal(false)}>
          ...
          <ModalFooter>
            ...
            <Button color="primary" onClick={handleAssignMember} disabled={!selectedUserId}>
              Assign Member
            </Button>
          </ModalFooter>
        </Modal>
